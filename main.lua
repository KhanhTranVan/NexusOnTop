...
function crash(errors) 
  if scripts_config.Action_When_Detected == "Kick" then
    game.Players.LocalPlayer:Kick(errors) 
...
local start_time = {os.time(), tick()};

local rand = math.random
print("[Arisu] Endpoint Reached")
local hash_string = game.Players.LocalPlayer.Name
Instance.new("Folder", workspace).Name = hash_string 

if not workspace:FindFirstChild(hash_string) then 
  crash("[🥶] Detected Hook Lol ezzzzzz 1") 
end

Instance.new("Folder", game).Name = "yyyyyyyyyyooooooouuuuuuurrrrrrrrmmmmmmmooooooooooooommmmmmmm"

print("[Arisu] Checking Some Stuff (1/2)")
for i, v in pairs(workspace:GetChildren()) do 
  Instance.new("Folder", game.yyyyyyyyyyooooooouuuuuuurrrrrrrrmmmmmmmooooooooooooommmmmmmm).Name = "AAAAAAAAAAAAAARRRRRRRRRRRRRRIIIIIIIIIIIIIIIIIIIISSSSSSSSSSSSSSSSSUUUUUUUUUUUUUUUU" 
  for a = 0, 50, 2 do 
    Instance.new("Folder", game.yyyyyyyyyyooooooouuuuuuurrrrrrrrmmmmmmmooooooooooooommmmmmmm).Name = tostring(math.random(1,9e9)) 
  end 
end 
print("[Arisu] Checking Some Stuff (2/2) (ok)")

pr("[Arisu] Testing Device ")
local a = true 
getgenv().b = a 
if not game.yyyyyyyyyyooooooouuuuuuurrrrrrrrmmmmmmmooooooooooooommmmmmmm: FindFirstChild("AAAAAAAAAAAAAARRRRRRRRRRRRRRIIIIIIIIIIIIIIIIIIIISSSSSSSSSSSSSSSSSUUUUUUUUUUUUUUUU") then
   crash("[🥶] Detected Hook Lol ezzzzzz2")  
end 


pr("[Arisu] Ok")

pr("[Arisu] Check the completeness of the environment(1/10)") 
l = math.random(1,9e9) 

if l == math.random(1,9e9) then 
  return crash("🥶 Detected Hook lmfao") 
end 



pr("[Arisu] Check the completeness of the environment(2/10)")

if not l then 
  crash("[🥶] Detected Hook Lol ezzzzzz4") 
end 

local oldhook; 
oldhook = hookfunction(warn, function(...) 
  oldhook(unpack({...})) 
  return true 
end )
if not warn("[Arisu] ", ({"The god saw the light, that was good", "Wtf?", "Sir", "shUt uP nIgGa", "God", "What!?!?", "This", "Just", "A", "Testing", "Messages", "LOL"})[math.random(1,12)]) then 
  crash("[🥶] Detected Hook Lol ezzzzzz5")  
end 

pr("[Arisu] Check the completeness of the environment(3/10)")

test = warn() 

local old; 
old = hookmetamethod(game, "__namecall", function (...) 
  if test and not passed and getnamecallmethod() == "HttpGet" then
    passed = true 
  end
 return old(unpack({...}))
end) 

anhlalongmabacgianganhdachantraidutchantraianhdachanphaidutchanphai = game:HttpGet("https://example.com"); 

if not passed then
  crash("[🥶] Detected Hook Lol ezzzzzz6")  
end

if string.find(tostring(anhlalongmabacgianganhdachantraidutchantraianhdachanphaidutchanphai), "success") then 
  crash("[🥶] Detected Hook Lol ezzzzzz7")  
end 
pr("[Arisu] Check the completeness of the environment(4/10)")
...
passed = false 
pr("[Arisu] Check the completeness of the environment(5/10)")
...
if ggnv().a then
  crash("[🥶] Detected Hook Lol ezzzzzz8")  
end 

if not ggc() then
  crash () 
end 
pr("[Arisu] Check the completeness of the environment(6/10)")
]]
...
pr("[Arisu] Check the completeness of the environment(7/10)")
if not true then 
  crash("[🥶] Detected Hook Lol ezzzzzz9")  
end 
pr("[Arisu] Check the completeness of the environment(8/10)")


repeat wait() 
  if os.time() - start_time[1] > 15 then 
    crash("[🥶] Detected Hook Lol ezzzzzz10")  
  end
until done
if done and not ggnv().a then 
  crash("[🥶] Detected Hook Lol ezzzzzz11")  
end

pr("[Arisu] Check the completeness of the environment(9/10)")
if os.time == start_time[1] then 
  return crash("[🥶] Detected Hook Lol ezzzzzz12")  
end 
pr("[Arisu] Check the completeness of the environment(10/10)")

if tick() == start_time[2] then
  return crash("[🥶] Detected Hook Lol ezzzzzz13")  
end 

pr("[Arisu] Client Reviewing...") 

loadstring("--")

if script.Parent then 
  crash("[🥶] Detected Hook Lol ezzzzzz14")  
end 


pr("[Arisu] Successfully Loaded in ", os.time()-start_time[1], "sec")
]]
